            ...
            while True:
                try:
                    if self.t > self.timeout:
                        ...
                        if self.file:
                            hyper_filename = f"{self.file}_params_{timestamp}_{unique_id}.json"
                            q_values_filename = f"{self.file}_q_values_small_{timestamp}_{unique_id}.json"
                            episode_log_filename = f"{self.file}_episode_log_small_{timestamp}_{unique_id}.json"
                        else:
                            hyper_filename = f"_params_{timestamp}_{unique_id}.json"
                            q_values_filename = f"q_values_small_{timestamp}_{unique_id}.json"
                            episode_log_filename = f"episode_log_small_{timestamp}_{unique_id}.json"

                        # Save Params
                        with open(os.path.join(results_folder, hyper_filename), 'w') as file:
                            json.dump(self.params, file)
                        print(f"Q-values saved successfully in {hyper_filename}!", flush=True)
